<?php
/**
 * ویرایش مبدا/مقصد
 */
require_once __DIR__ . '/../config/db.php';
require_once __DIR__ . '/../helpers/auth.php';

require_waybill_access();

$id = (int)($_GET['id'] ?? $_POST['id'] ?? 0);
$location = null;
$errors = [];
$regions = [];

try {
    $regions = db()->query('SELECT id, region_code, region_name FROM regions ORDER BY region_name')->fetchAll();
    $stmt = db()->prepare('SELECT * FROM locations WHERE id = ? LIMIT 1');
    $stmt->execute([$id]);
    $location = $stmt->fetch();
} catch (PDOException $e) {
    error_log('Location fetch error: ' . $e->getMessage());
}

if (!$location) {
    set_flash('danger', 'مکان مورد نظر یافت نشد.');
    header('Location: ' . BASE_URL . '/locations/list.php');
    exit;
}

$old = [
    'location_code' => $location['location_code'],
    'region_id'      => (string)$location['region_id'],
    'title'          => $location['title'],
    'geo_location'   => (string)$location['geo_location'],
];

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf()) {
        $errors[] = 'نشست شما منقضی شده است. لطفاً فرم را دوباره ارسال کنید.';
    } else {
        $old['location_code'] = trim((string)($_POST['location_code'] ?? ''));
        $old['region_id']     = (string)($_POST['region_id'] ?? '');
        $old['title']         = trim((string)($_POST['title'] ?? ''));
        $old['geo_location']  = trim((string)($_POST['geo_location'] ?? ''));

        if ($old['location_code'] === '' || mb_strlen($old['location_code']) > 20) {
            $errors[] = 'کد مکان الزامی است و باید حداکثر ۲۰ کاراکتر باشد.';
        }
        if (mb_strlen($old['title']) < 2) {
            $errors[] = 'عنوان مکان باید حداقل ۲ حرف باشد.';
        }
        $regionId = (int)$old['region_id'];
        if ($regionId <= 0) {
            $errors[] = 'انتخاب منطقه الزامی است.';
        }

        if (!$errors) {
            try {
                $stmt = db()->prepare('SELECT id FROM regions WHERE id = ? LIMIT 1');
                $stmt->execute([$regionId]);
                if (!$stmt->fetch()) {
                    $errors[] = 'منطقه انتخاب‌شده معتبر نیست.';
                }
            } catch (PDOException $e) {
                error_log('Region validate error: ' . $e->getMessage());
                $errors[] = 'خطایی در بررسی منطقه رخ داد.';
            }
        }

        if (!$errors) {
            try {
                $stmt = db()->prepare('SELECT id FROM locations WHERE location_code = ? AND id <> ? LIMIT 1');
                $stmt->execute([$old['location_code'], $location['id']]);
                if ($stmt->fetch()) {
                    $errors[] = 'مکان دیگری با این کد قبلاً ثبت شده است.';
                } else {
                    $stmt = db()->prepare(
                        'UPDATE locations SET location_code = ?, region_id = ?, title = ?, geo_location = ? WHERE id = ?'
                    );
                    $stmt->execute([
                        $old['location_code'],
                        $regionId,
                        $old['title'],
                        $old['geo_location'] !== '' ? $old['geo_location'] : null,
                        $location['id'],
                    ]);
                    set_flash('success', 'مکان با موفقیت ویرایش شد.');
                    header('Location: ' . BASE_URL . '/locations/list.php');
                    exit;
                }
            } catch (PDOException $e) {
                error_log('Update location error: ' . $e->getMessage());
                $errors[] = 'خطایی در ویرایش مکان رخ داد. لطفاً دوباره تلاش کنید.';
            }
        }
    }
}

$page_title = 'ویرایش مبدا/مقصد';
$active = 'locations';
require __DIR__ . '/../includes/header.php';
?>

<div class="mb-4">
  <h1 class="h4 fw-bold mb-1">ویرایش مبدا/مقصد</h1>
  <p class="text-muted small mb-0">اطلاعات مکان انتخاب‌شده را ویرایش کنید.</p>
</div>

<?php if ($errors): ?>
  <div class="alert alert-danger">
    <div class="d-flex align-items-center gap-2 fw-bold mb-2">
      <span class="iconify" data-icon="solar:danger-triangle-bold"></span> فرم دارای خطا است:
    </div>
    <ul class="mb-0 pe-4">
      <?php foreach ($errors as $err): ?><li><?= e($err) ?></li><?php endforeach; ?>
    </ul>
  </div>
<?php endif; ?>

<div class="card panel-card">
  <div class="card-body p-4">
    <form method="post" action="<?= BASE_URL ?>/locations/edit.php" novalidate>
      <?= csrf_field() ?>
      <input type="hidden" name="id" value="<?= e((string)$location['id']) ?>">
      <div class="row g-3">
        <div class="col-md-6">
          <label class="form-label" for="location_code">کد مکان</label>
          <div class="input-group">
            <span class="input-group-text"><span class="iconify" data-icon="solar:hashtag-square-bold"></span></span>
            <input type="text" class="form-control ltr-text" id="location_code" name="location_code" required maxlength="20"
                   value="<?= e($old['location_code']) ?>">
          </div>
        </div>
        <div class="col-md-6">
          <label class="form-label" for="region_id">منطقه</label>
          <div class="input-group">
            <span class="input-group-text"><span class="iconify" data-icon="solar:map-point-bold"></span></span>
            <select class="form-select" id="region_id" name="region_id" required>
              <option value="">— انتخاب کنید —</option>
              <?php foreach ($regions as $r): ?>
                <option value="<?= e((string)$r['id']) ?>" <?= (string)$r['id'] === $old['region_id'] ? 'selected' : '' ?>>
                  <?= e($r['region_name']) ?> (<?= e($r['region_code']) ?>)
                </option>
              <?php endforeach; ?>
            </select>
          </div>
        </div>
        <div class="col-md-6">
          <label class="form-label" for="title">عنوان مکان</label>
          <div class="input-group">
            <span class="input-group-text"><span class="iconify" data-icon="solar:signpost-bold"></span></span>
            <input type="text" class="form-control" id="title" name="title" required minlength="2"
                   value="<?= e($old['title']) ?>">
          </div>
        </div>
        <div class="col-md-6">
          <label class="form-label" for="geo_location">مختصات جغرافیایی (اختیاری)</label>
          <div class="input-group">
            <span class="input-group-text"><span class="iconify" data-icon="solar:global-bold"></span></span>
            <input type="text" class="form-control ltr-text" id="geo_location" name="geo_location"
                   value="<?= e($old['geo_location']) ?>">
          </div>
        </div>
      </div>

      <div class="d-flex gap-2 mt-4">
        <button type="submit" class="btn btn-primary d-flex align-items-center gap-2">
          <span class="iconify" data-icon="solar:diskette-bold"></span> ذخیره تغییرات
        </button>
        <a href="<?= BASE_URL ?>/locations/list.php" class="btn btn-outline-secondary">انصراف</a>
      </div>
    </form>
  </div>
</div>

<?php require __DIR__ . '/../includes/footer.php'; ?>
