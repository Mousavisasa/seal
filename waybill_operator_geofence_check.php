 ...
 * این توکن یک‌بارمصرف و مخصوص همین عملیات است (ساخته‌شده در operator_waybills_public.php
 * با helpers/tokens.php::create_operator_action_token) و نقش (origin/destination) +
 * شناسه بارنامه از داخل خودِ توکن استخراج می‌شود.
 *
 * این صفحه خودش هیچ تغییری در بارنامه ایجاد نمی‌کند؛ فقط موقعیت فعلی متصدی
 * را می‌گیرد و با محدودهٔ جغرافیایی مکان مبدا یا مقصد (بسته به نقش متصدی)
 * مقایسه می‌کند. تایید نهایی همیشه با کلیک دستی کاربر انجام می‌شود و با همین
 * توکن به وب‌سرویس api/operator_action.php درخواست می‌زند.
 */
...
if ($operator && $waybillId > 0) {
    try {
        ...
        } elseif (!empty($waybill[$confirmColumn])) {
...
<title>بررسی حصار جغرافیایی — تایید حضور متصدی <?= e($roleLabel) ?> | <?= e(APP_NAME) ?></title>
        ...
        <div class="row g-3">
          ...
          <div class="col-md-6 text-md-end">
            ...
            <div class="small text-muted mt-2">
              ...
              مکان هدف برای تایید حضور: <span class="fw-bold"><?= e($targetTitle) ?></span>
            </div>
          </div>
        </div>
      </div>
