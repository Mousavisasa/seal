 ...
 *
 * برای هر بارنامه، دکمهٔ مناسب (تایید حضور / تایید بارگیری / تایید تحویل)
 * کاربر را با یک توکن یک‌بارمصرف مخصوص همان عملیات
 * (helpers/tokens.php::create_operator_action_token برای تایید حضور، یا
 * helpers/tokens.php::create_operator_approve_token برای تایید بارگیری/تحویل)
 * به waybill_operator_geofence_check.php می‌فرستد تا پیش از ثبت نهایی، حصار
 * جغرافیایی بررسی شود.
 */
        ...
        <?php if ($waybills): ?>
            <div class="row g-3">
                <?php foreach ($waybills as $w): ?>
                    ...
                    $approveType   = null; // 'loading' یا 'delivery'
                    $actionLabel   = null;
                                ...
                                <div class="mt-auto pt-2 d-flex gap-2">
                                    <?php if ($approveType): ?>
                                        <div class="flex-fill d-grid gap-2">
                                            <?php $opApproveToken = create_operator_approve_token((int)$operator['id'], $approveType, (int)$w['id']); ?>
                                            <a href="<?= BASE_URL ?>/waybill_operator_geofence_check.php?token=<?= e(rawurlencode($opApproveToken)) ?>&myseal=<?= e(rawurlencode((string)$w['waybill_number'])) ?>"
                                    ...
                                    <?php else: ?>
                                        <div class="flex-fill d-grid gap-2">
                                            ...
                                            <a href="<?= BASE_URL ?>/waybill_operator_geofence_check.php?token=<?= e(rawurlencode($opToken)) ?>&myseal=<?= e(rawurlencode((string)$w['waybill_number'])) ?>"
