<?php
/**
 * صفحه عمومی مشاهده و تایید تحویل بارنامه‌های پایان‌پیمایش برای متصدی مقصد
...
if (!$operator && $token !== '') {
    try {
        ...
        error_log('Operator delivery token validate error: ' . $e->getMessage());
    ...
    if ($submittedUsername === '' || $password === '') {
        ...
        try {
            ...
            if (!$user || !password_verify($password, $user['password'])) {
                ...
                header('Location: ' . BASE_URL . '/operator_delivery_public.php?token=' . rawurlencode($newToken));
                exit;
            }
        } catch (PDOException $e) {
            error_log('Operator delivery auth error: ' . $e->getMessage());
            $errors[] = 'خطایی رخ داد. لطفاً بعداً تلاش کنید.';
        }
    }
}

// ---------- دریافت بارنامه‌های پایان‌پیمایش ----------
if ($operator) {
    try {
        $stmt = db()->prepare(
            "SELECT w.*, ol.title AS origin_title, dl.title AS destination_title,
             ...
             WHERE w.destination_operator_user_id = ? AND w.send_status = 'پایان پیمایش'
             ORDER BY w.id DESC"
        );
        $stmt->execute([$operator['id']]);
        $waybills = $stmt->fetchAll();
    } catch (PDOException $e) {
        error_log('Operator delivery fetch error: ' . $e->getMessage());
...
<title>تایید تحویل | <?= e(APP_NAME) ?></title>
                    ...
                    <div class="card panel-card border-0 shadow-sm">
                        <div class="card-body p-4">
                            <h3 class="mb-4 text-center">
                                <span class="iconify" data-icon="solar:delivery-bold" style="font-size: 28px;"></span>
                                ورود متصدی مقصد
                            </h3>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        <h2>
                            <span class="iconify" data-icon="solar:delivery-bold"></span>
                            بارنامه‌های انجام‌شده و منتظر تحویل
                        </h2>
            ...
            <?php else: ?>
                <div class="card panel-card border-0 shadow-sm">
                    <div class="card-body text-center text-muted p-5">
                        ...
                        <p>بارنامه‌ای برای تحویل وجود ندارد.</p>
                    </div>
                </div>
            <?php endif; ?>
        <?php endif; ?>
...
document.querySelectorAll('.approve-btn').forEach(btn => {
    btn.addEventListener('click', async (e) => {
        ...
        if (!confirm('آیا مطمئن هستید که می‌خواهید این بارنامه را به عنوان تحویل‌شده ثبت کنید؟')) {
            return;
        }
        ...
        try {
            const response = await fetch('<?= BASE_URL ?>/api/operator_approve_delivery.php', {
                method: 'POST',
                headers: {
            ...
            if (data.success) {
                alert('بارنامه با موفقیت به عنوان تحویل‌شده ثبت شد!');
                location.reload();
            } else {
